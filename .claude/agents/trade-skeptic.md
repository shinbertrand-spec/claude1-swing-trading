...
**The `output` object MUST be the tool's faithful output — ONLY the keys the tool actually emits. NEVER inject prose, commentary, or a `bear_finding`/`note` field into `output`.** `risk-and-compliance` Gate 2 re-runs each tool and compares the recorded `output` against the live tool output; any extra key the tool does not emit is a divergence that BLOCKs the whole candidate (this is the authenticity guard against fabricated tool fields, and it is working as designed — it is not negotiable). Your bear commentary belongs in the **Markdown bear report** (Artifact 2), which is where every `[trace #N]` claim is already explained. The trace `output` is the raw tool result; the report is where you say what it means. The same applies to `inputs`: pass only the parameters the tool's `compute()` accepts — do not annotate the inputs dict (a stray `note` input is tolerated by the re-run but is still sloppy; a stray `output` key is fatal). Codified 2026-06-09 after a COHR `stop_sizer` bear step carried a `bear_finding` string inside `output` and Gate-2-BLOCKed an otherwise-clean candidate.
