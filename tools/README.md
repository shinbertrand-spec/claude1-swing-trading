...
| [`thematic_portfolio/put_overlay.py`](thematic_portfolio/put_overlay.py) | Loop 3 short-overlay recommendation. Computes SA LP barbell ratio (put_complex_notional / long_book_value), caps cash-raise at 50% reduction; NVDA-only 1-2% portfolio-insurance leg with hard refusals on SMH puts (Week 1c spread quality) + SA LP put-complex replication (institutional-only). Light Street chip-exposure cross-reference signal. | Loop 1's `short_overlay_bias_flag` block when `fired: true` |
...
Not yet built: X-timeline fetcher (twitterapi.io, blocked on Bertrand account creation), podcast RSS + Whisper transcription, press feed RSS parsers, Tier 3 real-world signal compilers, kill-switch Process B monitor.
